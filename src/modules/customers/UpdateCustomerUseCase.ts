...
export class UpdateCustomerUseCase {
  async execute({
    ...
    logger.info(`Iniciando processo de atualização do cliente ${id}`);
    const customerRepository = CustomerRepository();

    const customer = await customerRepository.findOne({ id });

    if (!customer) {
      logger.warn("Cliente não existe.");
    ...
    customer.state_registration = state_registration
      ? state_registration
    ...
    logger.info(`Cliente "${customer.full_name}" atualizado com sucesso.`);
    return customer;
  }
}
